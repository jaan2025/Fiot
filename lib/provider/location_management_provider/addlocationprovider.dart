...
final addLocationNotifier =
StateNotifierProvider<AddLocationProvider, AddUserState>((ref) {
  return AddLocationProvider(ref);
});

class AddLocationProvider extends StateNotifier<AddUserState> {
  Ref ref;

  AddLocationProvider(this.ref)
      : super(AddUserState(false, const AsyncLoading(), 'initial'));

  addLocation(var addLocation) async {
    state = _loading();
